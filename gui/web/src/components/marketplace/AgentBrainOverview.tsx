...
import { RoutingStrategySelector } from '@/components/brains/RoutingStrategySelector';
import { LlmCard } from '@/components/brains/LlmCard';
import { LlmEditor } from '@/components/brains/LlmEditor';

interface AgentBrainOverviewProps {
  agentId: string;
  agentName: string;
  onBack: () => void;
}

export function AgentBrainOverview({ agentId, agentName, onBack }: AgentBrainOverviewProps) {
  ...
  const fetchBrain = async () => {
    const data = await api.getAgentBrain(agentId);
    setBrain(data);
    setLoading(false);
  };

  useEffect(() => { fetchBrain(); }, [agentId]);

  const handleStrategyUpdate = async (data: { routingStrategy?: string; fallbackLlmId?: string }) => {
    const updated = await api.updateAgentBrain(agentId, data);
    setBrain(updated);
  };

  const handleAddLlm = async (data: any) => {
    await api.addAgentBrainLlm(agentId, data);
    setAdding(false);
    fetchBrain();
  };
  ...
  const handleUpdateLlm = async (data: any) => {
    if (!editingId) return;
    await api.updateAgentBrainLlm(agentId, editingId, data);
    setEditingId(null);
    fetchBrain();
  };

  const handleDeleteLlm = async (llmId: string) => {
    await api.deleteAgentBrainLlm(agentId, llmId);
    fetchBrain();
  };

  const handleTestLlm = async (llmId: string) => {
    setTestResult(null);
    const result = await api.testAgentBrainLlm(agentId, llmId);
    setTestResult({ llmId, message: result.message, success: result.success });
  };

  const handleToggle = async (llmId: string, enabled: boolean) => {
    await api.updateAgentBrainLlm(agentId, llmId, { enabled });
    fetchBrain();
  };
  ...
  const handleMoveUp = async (index: number) => {
    ...
    await api.reorderAgentBrainLlms(agentId, ids);
    fetchBrain();
  };
