...
export class ServiceNowClient {
     ...
     */
    async updateIncident(numberOrSysId, updates) {
        ...
        if (isNumber) {
            ...
            if (!sysId)
                throw new Error(`Incident ${numberOrSysId} not found`);
     ...
     */
    async getIncident(numberOrSysId) {
        const isNumber = numberOrSysId.startsWith('INC');
        const endpoint = isNumber
            ? `/incident?sysparm_query=number=${numberOrSysId}`
            : `/incident/${numberOrSysId}`;
...
export function activate(context) {
    ...
    return {
        tools,
        handlers: {
            servicenow_create_incident: async (input) => {
            ...
            servicenow_update_incident: async (input) => {
            ...
            servicenow_create_change: async (input) => {
